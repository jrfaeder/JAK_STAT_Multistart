"""
Debug test to understand why plot_fit produces identical plots.
"""
...
# Test with 2 very different parameter sets
print("Testing plot_fit with 2 different parameter sets:\n")
...
for idx in [0, 100]:
    ...
    print(f"Final total_pS1 (IL6=1, IL10=0) from simulate_conditions: {final_pS1:.6e}")

    # Now call plot_fit to see if it gets the same results
    # Don't actually plot, just get the simulation results
    print("\nCalling plot_fit...")

    # Manually replicate what plot_fit does
    param_dict = estimator.vector_to_params(param_vector)
    results2 = estimator.simulate_conditions(param_vector)
    final_pS1_2 = results2[cond_id]['total_pS1'][-1]
    print(f"Final total_pS1 (IL6=1, IL10=0) from second simulate_conditions: {final_pS1_2:.6e}")

    # Check if they match
    if abs(final_pS1 - final_pS1_2) < 1e-10:
        print("✓ Results match!")
    else:
        print(f"✗ Results differ! Difference: {abs(final_pS1 - final_pS1_2):.6e}")

print("\n\nNow testing if simulate_conditions is idempotent...")
print("Calling simulate_conditions 3 times with same parameters:")
...
for i in range(3):
    ...
    cond_id = conditions_subset.iloc[0]['conditionId']
    final_pS1 = results[cond_id]['total_pS1'][-1]
    print(f"  Call {i+1}: {final_pS1:.12e}")
